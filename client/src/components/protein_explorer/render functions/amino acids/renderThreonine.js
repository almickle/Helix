...
export default function renderThreonine (residue, scene) {

    const vectorAtoms = residue.map((atom) => new Vector3(atom.x, atom.y, atom.z))
    
    const main = residue.filter((atom) => atom.atom_type === 'N' || atom.atom_type === 'CA' || atom.atom_type === 'CB' || atom.atom_type === 'CG2').map((atom) => new Vector3(atom.x, atom.y, atom.z))
    ...
    const hydroxy = residue.filter((atom) => atom.atom_type === 'CB' || atom.atom_type === 'OG1').map((atom) => new Vector3(atom.x, atom.y, atom.z))

    MeshBuilder.CreateTube('main', {path: main, radius: 0.2}, scene)
    MeshBuilder.CreateTube('main', {path: carbonyl, radius: 0.2}, scene)
    MeshBuilder.CreateTube('main', {path: hydroxy, radius: 0.2}, scene)
